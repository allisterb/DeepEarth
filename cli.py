...
def print_logo():
    ...
    print(fig.renderText('DeepEarth') + 'v0.1\n')
    
parser = argparse.ArgumentParser()
parser.add_argument("--debug", help="Enable debug-level logging.", action="store_true")
parser.add_argument("--test", help="Test if DeepEarth can read from the default camera device.", action="store_true")
...
if args.test:
    ...
    while not kbinput.KBINPUT:
        ...
        if not ret:
            ...
            break
        cv.imshow(f'DeepEarth Test: {width}x{height} {fps}fps.', frame)
        if cv.waitKey(1) != -1:
            break
    cap.release()
